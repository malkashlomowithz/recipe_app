...
import '../RecipeComponents/AddRecipe.css'
...
function UpdateRecipe(props) {
 ...
 //get all the details of the recipe
...
// check user filed in the fieleds
        if( state.title.length && state.category.length && state.ingredients.length && state.preparations.length){
            ...
            axios.put(`${API_BASE_URL_RECIPES}`, recipe)
            .then(res => {
    ...
    return(
        <div className="card col-12 col-lg-4 mt-2 hv-center addRecipe">
            {data.map(item => ( 
        <form key = {item._id}>
            <h4 className = "cardTitle">Update your recipe:</h4>
